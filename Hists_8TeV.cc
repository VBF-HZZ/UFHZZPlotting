...
int main(int argc, char* argv[]){
  ...
  if (argc != 9){
    cout << "Usage: ./makeHists8TeV.exe <fileDir> <plotXlow> <plotXhigh> <binSize> <plotXlow_zoom> <plotXhigh_zoom> <binSize_zoom> <print data>" << endl;
    return 1;
  }
  ...
  TString fileDir = "../Histogramming_8TeV/rootFiles_Legacy_dataMC/";
  TString fileDir_7 = "../Histogramming/rootFiles_Legacy_dataMC/";
  TString treeName1 = "AnaAfterHlt/passedEvents";
  TString treeName = "passedEvents_dataMC";

  // ------------- Histograms -------------- //

  //lumi
  double lumi = 19.63;//7.41;
  double sqrts = 8;
  ...
  Double_t plotYmax_zoom = 25;
  TString yAxisLabel_zoom = (TString)"Events / "+Form("%.0f",binSize_zoom)+" GeV";

  //reducible background
  Double_t  nBgEvents_4mu = 3.0;
  Double_t  nBgEvents_4e = 4.9;
  Double_t  nBgEvents_2e2mu = 7.1;
  ...
  Collection *Cggzz4l    = new Collection("ggZZ4l",fileDir_7+"/ggZZ_4l.root"       ,treeName,lumi*1.18,false,massZ1Cut,massZ2Cut,ptMuCut, ptElCut,m4lCut);
  Collection *Cggzz2l2l  = new Collection("ggZZ2l2l",fileDir_7+"/ggZZ_2e2mu.root"  ,treeName,lumi*1.18,false,massZ1Cut,massZ2Cut,ptMuCut, ptElCut,m4lCut);
  Collection *Cdata      = new Collection("Data",fileDir+"/Data.root"            ,treeName1,1,true,massZ1Cut,massZ2Cut,ptMuCut, ptElCut,m4lCut);
  //Collection *Cdata      = new Collection("Data","../Histogramming_8TeV/rootFiles_Moriond_old/Data.root"            ,treeName1,1,true,massZ1Cut,massZ2Cut,ptMuCut, ptElCut,m4lCut);
  ...
  if(printData)
    {
      //fileDir = "../Histogramming_8TeV/rootFiles_Moriond_old/";
      helper->printData(fileDir+"/Data.root",treeName1,       txtDir+"/SChannelEvents_8TeV.txt",      40.0, 4.0, ptMuCut,ptElCut, 80.0,  100.0);
      helper->printDataSync(fileDir+"/Data.root",treeName1,   txtDir+"/SyncEvents_8TeV.txt",          40.0, 12.0,ptMuCut,ptElCut, 70.0,  1000.0);
      helper->printData(fileDir+"/Data.root",treeName1,      txtDir+"/PassedEvents_8TeV.txt",         40.0, 12.0,ptMuCut,ptElCut, 70.0, 1000.0);
      helper->printDataShort(fileDir+"/Data.root",treeName1, txtDir+"/PassedEvents_short_8TeV.txt",   40.0, 12.0,ptMuCut,ptElCut, 100.0, 1000.0);
      helper->printDataTex(fileDir+"/Data.root",treeName1,       txtDir+"/PassedEvents_8TeV.tex",     40.0, 12.0,ptMuCut,ptElCut, 100.0, 1000.0);
      
      //helper->printDataErr(fileDir+"/Data.root",treeName,       txtDir+"/EventErrors.txt",       40.0, 12.0, 70.0, 170.0);
      //helper->printFourMomenta(fileDir+"/Data.root",treeName,       txtDir+"/EventFourMomenta_8TeV.txt",  40.0, 12.0, 115.0, 135.0);
      //helper->printData(fileDir+"/Data.root","AnaAfterHlt/passedEvents",      txtDir+"/PassedEvents_forTiz.txt",       40.0, 12.0, 121.5, 130.5);
    }
  ...
  helper->printCollectionYield(MCZZ_noTaus, out, 40., 12., 100., 1000.);
  helper->printCollectionYield(MCZZ_taus, out, 40., 12., 100., 1000.);
  helper->printCollectionYield(MCggZZ, out, 40., 12., 100., 1000.);
  helper->printCollectionYield(MCH126, out, 40., 12., 100., 1000.);
  helper->printCollectionYield(MCH350, out, 40., 12., 100., 1000.);
  out << "----------------" << endl;
  out << "Z4L Fit"          << endl;
